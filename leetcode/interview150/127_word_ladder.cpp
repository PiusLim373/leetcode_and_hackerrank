...
int ladderLengthTimedOut(std::string beginWord, std::string endWord, std::vector<std::string>& wordList)
{
  if (std::find(wordList.begin(), wordList.end(), endWord) == wordList.end())
    return 0;
    ...
    for (int i = 0; i < curr.size(); i++)
    {
      for (auto alphabet : alphabets)
      {
        if (alphabet != curr[i])
        {
          ...
          if (temp == endWord)
            return count + 1;
          if (std::find(wordList.begin(), wordList.end(), temp) != wordList.end() && !visited[temp])
          {
